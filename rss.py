...
from database import create_database, save_news_to_db, categorize_news, remove_duplicates
...
def parse(feed_url, source_name):
    feed = feedparser.parse(feed_url)
    articles = []
    for e in feed.entries:
        # если даты нет — пропускаем, чтобы не «забивать» сегодняшней
...
def fetch_rss_news():
    ...
    # оставляем статьи, где встречаются фамилии
    key_words = ["trump", "putin", "xi jinping", " xi "]
    filtered = [
        a for a in all_arts
        if any(k in (a["title"] + " " + a["description"] + " " + a["content"]).lower()
               for k in key_words)
    ]
